...
fun CatDetailInfo(cat: CatInfo, onClick: (Boolean?) -> Unit) {
    Column(Modifier
        ...
        Image(painterResource(id = cat.picture), "Cat Name: ${cat.name}",
            Modifier.size(300.dp).padding(top = 30.dp),
            contentScale = ContentScale.Crop)
        Column(
            Modifier
                .padding(8.dp)
            ...
            Text("vaccineInjected: ${cat.vaccineInjected}", fontWeight= FontWeight.Bold, fontSize = 20.sp)
        }
        Row {
            Button(onClick = {
                if (!cat.isAdopted) {
